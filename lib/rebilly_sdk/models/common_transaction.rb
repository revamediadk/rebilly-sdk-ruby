  ...
  class CommonTransaction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::CommonTransaction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::CommonTransaction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      type_validator = EnumAttributeValidator.new('Object', ['3ds-authentication', 'authorize', 'capture', 'credit', 'refund', 'sale', 'setup', 'void'])
      return false unless type_validator.valid?(@type)
      status_validator = EnumAttributeValidator.new('Object', ['completed', 'conn-error', 'disputed', 'never-sent', 'offsite', 'partially-refunded', 'pending', 'refunded', 'sending', 'suspended', 'timeout', 'voided', 'waiting-approval', 'waiting-capture', 'waiting-gateway', 'waiting-refund'])
      return false unless status_validator.valid?(@status)
      result_validator = EnumAttributeValidator.new('Object', ['abandoned', 'approved', 'canceled', 'declined', 'unknown'])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('Object', ['3ds-authentication', 'authorize', 'capture', 'credit', 'refund', 'sale', 'setup', 'void'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Object', ['completed', 'conn-error', 'disputed', 'never-sent', 'offsite', 'partially-refunded', 'pending', 'refunded', 'sending', 'suspended', 'timeout', 'voided', 'waiting-approval', 'waiting-capture', 'waiting-gateway', 'waiting-refund'])
    ...
    def result=(result)
      validator = EnumAttributeValidator.new('Object', ['abandoned', 'approved', 'canceled', 'declined', 'unknown'])
